...
def send_report_email(to_addr: str, csv_path: str, team_name: str, date_str: str) -> None:
    ...
    *to_addr* may contain a single address or multiple comma-separated
    ...
    body = (
        ...
        f"  metrics_interactions_28d        Total user-initiated prompts across all Copilot features\n"
        f"  metrics_completions_28d         Number of times Copilot generated code for the user\n"
        ...
        f"  metrics_loc_suggested_by_language_total_28d  Per-language breakdown of LOC proposed across ALL features\n"
        f"                                               (inline ghost-text [code_completion] + Chat Ask/inline\n"
        f"                                               [chat_panel_ask_mode, chat_inline, chat_panel_unknown_mode] +\n"
        f"                                               Edit mode [chat_panel_edit_mode, edit, edit_mode] +\n"
        f"                                               Agent/Plan/Custom [chat_panel_agent_mode, chat_panel_plan_mode,\n"
        f"                                               chat_panel_custom_mode, agent, agent_edit]),\n"
        f"                                               sorted by volume descending (e.g. 'Python 420, TypeScript 310').\n"
        f"                                               Note: agent_edit contributes 0 to loc_suggested (direct file\n"
        f"                                               writes bypass the suggestion UI).\n"
        f"  metrics_loc_added_by_language_total_28d      Per-language breakdown of LOC applied (accepted) across ALL\n"
        f"                                               features (inline + chat + edit + agent, including agent_edit\n"
        f"                                               direct file writes), sorted by volume descending\n"
        f"                                               (e.g. 'Python 380, TypeScript 280').\n"
        f"  metrics_loc_suggested_by_language_inline_28d Per-language breakdown of LOC proposed via inline ghost-text\n"
        f"                                               completions only (code_completion feature). Chat, Edit, and\n"
        f"                                               Agent are excluded.\n"
        f"  metrics_loc_added_by_language_inline_28d     Per-language breakdown of LOC accepted from inline ghost-text\n"
        f"                                               completions only (code_completion feature).\n"
        f"  metrics_loc_suggested_by_language_agent_28d  Per-language breakdown of LOC proposed by Agent/Plan/Custom\n"
        f"                                               mode features (chat_panel_agent_mode, chat_panel_plan_mode,\n"
        f"                                               chat_panel_custom_mode, agent, agent_edit).\n"
        f"                                               agent_edit contributes 0 (direct file writes have no\n"
        f"                                               suggestion UI; loc_suggested_to_add_sum = 0 in the API).\n"
        f"  metrics_loc_added_by_language_agent_28d      Per-language breakdown of LOC applied in Agent/Plan/Custom\n"
        f"                                               mode (chat panel code blocks + agent_edit direct file writes).\n"
        f"\n"
        f"─────────────────────────────────────────\n"
