...
def send_report_email(to_addr: str, csv_path: str, team_name: str, date_str: str) -> None:
    ...
    *to_addr* may contain a single address or multiple comma-separated
    ...
    body = (
        ...
        f"  metrics_loc_suggested_28d       LOC Copilot proposed (= inline + chat + edit + agent; raw loc_suggested_to_add_sum from API; agent_edit contributes 0 as it writes directly to files)\n"
        ...
        f"  metrics_loc_suggested_agent_28d       LOC proposed by Agent/Plan mode (raw loc_suggested_to_add_sum\n"
        f"                                        from the API for all agent features). agent_edit contributes\n"
        f"                                        0 because it writes directly to files and has no suggestion UI.\n"
        ...
        f"─────────────────────────────────────────\n"
        f"WHY loc_suggested_agent CAN BE 0 FOR agent_edit\n"
        f"─────────────────────────────────────────\n"
        ...
        f"bypassing the suggestion UI.  This 0 is reported faithfully — no proxy is\n"
        f"applied.  Chat-panel agent features (chat_panel_agent_mode, chat_panel_plan_mode,\n"
        f"chat_panel_custom_mode) do return real loc_suggested_to_add_sum values.\n"
        f"As a result, heavy agent_edit use can make loc_added exceed loc_suggested,\n"
        f"which is expected and not a data error.\n"
    )
