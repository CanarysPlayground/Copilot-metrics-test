...
def send_report_email(to_addr: str, csv_path: str, team_name: str, date_str: str) -> None:
    ...
    *to_addr* may contain a single address or multiple comma-separated
    ...
    body = (
        ...
        f"─────────────────────────────────────────\n"
        f"WHY metrics_loc_suggested_28d CAN BE LESS THAN metrics_loc_added_28d\n"
        f"─────────────────────────────────────────\n"
        f"metrics_loc_suggested_28d sums loc_suggested_to_add_sum across all features\n"
        f"(inline, Chat, Edit, and Agent). However, the GitHub Copilot Metrics API does\n"
        f"NOT populate loc_suggested_to_add_sum for agent_edit — the feature that captures\n"
        f"direct file writes by Agent, Plan, and Edit modes. For agent_edit, the suggested\n"
        f"count is always 0 in the API response, even though Copilot actively wrote those\n"
        f"lines to disk.\n\n"
        f"metrics_loc_added_28d, by contrast, sums loc_added_sum for all features, including\n"
        f"agent_edit. Because agent file writes are counted in loc_added but not in\n"
        f"loc_suggested, heavy Agent/Plan mode usage will make metrics_loc_added_28d exceed\n"
        f"metrics_loc_suggested_28d. This is an API limitation, not a data error.\n\n"
        f"Note: metrics_loc_suggested_agent_28d uses loc_added_sum + loc_deleted_sum as the\n"
        f"best available proxy for lines Copilot proposed in Agent/Plan mode, since the true\n"
        f"suggested count is unavailable from the API.\n"
    )
